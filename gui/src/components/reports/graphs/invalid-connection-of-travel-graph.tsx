  ...
  if (active && payload && payload.length) {
    ...
    return (
      <Box sx={{ backgroundColor: 'white', border: '1px solid #ccc', padding: '10px' }}>
        ...
        <Typography variant="body2">Event Count: {obj.eventCountInvalid}</Typography>
      </Box>
    );
  }
  return null;
...
const InvalidConnectionOfTravelGraph: React.FC<InvalidConnectionOfTravelGraphProps> = ({ data }) => {
  ...
  const processedData = Object.values(aggregatedData).filter(item => item.eventCountInvalid > 0);
  ...
  return (
    <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%', height: 'auto' }}>
      <Box>
        <Typography variant="h6" align="center" sx={{ mt: 2 }}>Invalid Connection of Travel</Typography>
        <BarChart
          width={750}
          height={450}
        ...
        >
          ...
          <Bar dataKey="eventCountInvalid" fill={reportColorPalette[0]} />
        </BarChart>
      </Box>
    </Box>
  );

  ...
  if (active && payload && payload.length) {
    ...
    return (
      <Box sx={{ backgroundColor: 'white', border: '1px solid #ccc', padding: '10px' }}>
        ...
        <Typography variant="body2">Red Time: {roundToTwoDecimals(obj.redTime)}s, {roundToTwoDecimals(obj.red)}%</Typography>
        <Typography variant="body2">Yellow Time: {roundToTwoDecimals(obj.yellowTime)}s, {roundToTwoDecimals(obj.yellow)}%</Typography>
        <Typography variant="body2">Green Time: {roundToTwoDecimals(obj.greenTime)}s, {roundToTwoDecimals(obj.green)}%</Typography>
        <Typography variant="body2">Dark Time: {roundToTwoDecimals(obj.darkTime)}s, {roundToTwoDecimals(obj.dark)}%</Typography>
      </Box>
    );
  }
  return null;
...
const SignalGroupStopGraph: React.FC<SignalGroupStopGraphProps> = ({ data }) => {
  const formattedData = data.map(group => {
    const total = Math.max(group.timeStoppedOnRed + group.timeStoppedOnYellow + group.timeStoppedOnGreen + group.timeStoppedOnDark, 1) / 100;
    return {
  ...
  return (
    <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%', height: 'auto' }}>
      <Box>
        <Typography variant="h6" align="center" sx={{ mt: 2 }}>Signal Group Stop Line Assessment</Typography>
        <BarChart
          width={750}
          height={450}
          data={formattedData}

...
const LaneDirectionDistanceGraph: React.FC<LaneDirectionDistanceGraphProps> = ({ data, getInterval, distanceTolerance }) => {
  ...
  return (
    <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%', height: 'auto' }}>
      <Box>
        <Typography variant="h6" align="center" sx={{ mt: 2 }}>Median Distance Deviation from Lane Centerline</Typography>
        <BarChart
          width={750}
          height={450}
          data={numericData}
        ...
        >
          ...
          <ReferenceLine x={-cmToFeet(distanceTolerance)} stroke={reportColorPalette[0]} strokeDasharray="3 3" />
          <ReferenceArea x1={cmToFeet(distanceTolerance)} x2={maxX + 0.5} fill={reportColorPalette[3]} fillOpacity={0.1} />
        ...
        {data.length > 0 && (
          <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2 }}>
            ...
            <Typography variant="body2" sx={{ mx: 1 }}>Min: {min} ft</Typography>
            <Typography variant="body2" sx={{ mx: 1 }}>Max: {max} ft</Typography>
          </Box>
        )}
      </Box>
    </Box>

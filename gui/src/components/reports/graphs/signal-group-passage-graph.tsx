  ...
  if (active && payload && payload.length) {
    ...
    return (
      <Box sx={{ backgroundColor: 'white', border: '1px solid #ccc', padding: '10px' }}>
        ...
        <Typography variant="body2">Red Events: {roundToInteger(obj.redEvents)}, {roundToTwoDecimals(obj.red)}%</Typography>
        <Typography variant="body2">Yellow Events: {roundToInteger(obj.yellowEvents)}, {roundToTwoDecimals(obj.yellow)}%</Typography>
        <Typography variant="body2">Green Events: {roundToInteger(obj.greenEvents)}, {roundToTwoDecimals(obj.green)}%</Typography>
        <Typography variant="body2">Dark Events: {roundToInteger(obj.darkEvents)}, {roundToTwoDecimals(obj.dark)}%</Typography>
      </Box>
    );
  }
  return null;
...
const SignalGroupPassageGraph: React.FC<SignalGroupPassageGraphProps> = ({ data }) => {
  ...
  return (
    <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%', height: 'auto' }}>
      <Box>
        <Typography variant="h6" align="center" sx={{ mt: 2 }}>Signal Group Passage Events</Typography>
        <BarChart
          width={750}
          height={450}
          data={formattedData}
